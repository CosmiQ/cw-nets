...
# Setting Certificate Location for Ubuntu/Mac OS locations
# (Rasterio looks for certs in centos locations)
...
class largeGeoTiff(Dataset):  # TODO: DEFINE IN AN APPROPRIATE LOCATION
    ...
    def __init__(self, raster_path,
        ...
        rasterBounds, dst_profile = base_tools.get_processing_details(
            self.raster_path, smallExample=sample)
        ...
        self.cells_list = base_tools.generate_cells_list_dict(
            rasterBounds, self.cell_size_meters, self.stride_size_meters,
            self.tile_size_pixels, quad_space=quad_space
            )
        self.cells_list = self.cells_list[0]
        if self.testing:
            with rasterio.open("test.tif", "w", **dst_profile) as dst:
                self.cells_list = [window for ij, window
                                   in dst.block_windows()]
    ...
    def __getitem__(self, idx):
        ...
        if self.testing:
            ...
            tile, mask, window, window_transform = main.tile_utm(
                src_ras, ll_x, ll_y, ur_x, ur_y, indexes=None,
                tilesize=self.tile_size_pixels, nodata=None, alpha=None,
...
def pad(img, pad_size=32):
    ...
    divisible by 32 (network requirement)
    if pad = True:
...
def ternaus_transform(sample):  # TODO: MOVE TRANSFORMS TO THEIR OWN SUBMODULE
    """sample = {'tile': tile,
                  'mask': mask,
                 'window': window,
                 'window_transform': window_transform}
